    ...
    with (
        ...
        while processor_manager.is_running:
            receiver_client.receive_messages(
                MAX_BATCH_SIZE, lambda message: _process_message(message, hl7_sender_client, event_logger)
            )


def _process_message(
    message: ServiceBusMessage, hl7_sender_client: HL7SenderClient, event_logger: EventLogger
) -> bool:
