...
class TestRemoveTimezoneFromDatetime(unittest.TestCase):
    ...
    def test_remove_timezone_for_various_datetime_formats(self) -> None:
        test_cases = [
    ...
    def test_datetime_without_timezone_remains_unchanged(self) -> None:
        test_cases = ["20250819115447", "20000101"]

        for input_datetime in test_cases:
            with self.subTest(input_datetime=input_datetime):
                result = remove_timezone_from_datetime(input_datetime)

                self.assertEqual(result, input_datetime)
    ...
    def test_invalid_length_after_timezone_removal(self) -> None:
        ...
        for input_datetime in invalid_length_cases:
            with self.subTest(input_datetime=input_datetime):
                with self.assertRaises(ValueError) as context:
                    remove_timezone_from_datetime(input_datetime)
    ...
    def test_invalid_format_non_numeric_characters(self) -> None:
        ...
        for input_datetime in invalid_format_cases:
            with self.subTest(input_datetime=input_datetime):
                with self.assertRaises(ValueError) as context:
                    remove_timezone_from_datetime(input_datetime)
