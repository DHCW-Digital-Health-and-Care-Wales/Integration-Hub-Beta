...
### Issue #1: Incorrect Profile Name
**Problem**: Initially attempted to use `phe-to-mpi` which doesn't exist in the `docker-compose.yml`.

**Resolution**: Checked available profiles in `docker-compose.yml` and identified the correct profile name is `phw-to-mpi` (Public Health Wales to MPI).
...
### Issue #2: `just` Command Uses `docker` Not `podman-compose`
...
### Issue #3: Missing CA Certificates (`.crt` files)
...
### Issue #4: Missing `phw-hl7-transformer` Image
