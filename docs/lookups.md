...
### Option D — Dedicated lookup REST server (separate container)

Considered for completeness: instead of each transformer talking to Cosmos directly via a shared

library, a standalone service (e.g. `lookups/hl7_lookup_server`, following the existing

`hl7_rest_server`/`buswatch` pattern of a small FastAPI/Flask app in its own container) owns the

Cosmos connection and caching, and transformers call it over HTTP —

`GET /lookups/{table_name}/{key}`.

- **Pros**: centralises the Cosmos client, cache, and retry logic in one place instead of a library

vendored into every transformer image — a cache/config change ships once instead of via a

coordinated redeploy of every transformer; one process's cache is shared by all callers instead of

each transformer keeping a cold, per-instance cache; keeps `azure-cosmos` and `cachetools` out of

every transformer's dependency tree; the Cosmos data-plane RBAC role only needs to be granted to

one identity (the lookup server's) rather than every transformer's.

- **Cons**: a new network hop and failure mode per lookup call (DNS, TLS, connection resets, HTTP

timeouts) in the hot path of message transformation, on top of the Cosmos call itself; a new

service to build, containerise, deploy, scale, and monitor (health checks, autoscaling rules,

Container Apps ingress) for what is otherwise a narrow read path; the

[retry-then-hard-fail](#failure-handling-retry-then-hard-failure) semantics still apply, but now

have to be layered on an HTTP client as well as the Cosmos SDK call it wraps; introduces internal

service-to-service auth (managed identity token or network-level restriction via Container Apps

internal ingress) that the in-process library approach doesn't need; the caching benefit only

beats a shared library if there are enough transformer replicas hitting the same keys often enough

to offset the added hop — for the data volumes described in the spec (small-to-medium reference

tables), that crossover is unproven.

- **Fit**: would only clearly earn its cost if lookup tables grow large/numerous enough, or gain

enough independent consumers (e.g. the dashboard, or non-Python services), that a shared library

duplicated per-transformer becomes the bigger problem — not the case today.

- **Verdict**: rejected for now in favour of Option C's in-process library. Revisit if a second

non-transformer consumer of lookup data emerges, or if per-transformer Cosmos RBAC/dependency

sprawl becomes a real maintenance problem.
