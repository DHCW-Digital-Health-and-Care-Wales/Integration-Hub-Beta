...
clear_feedback: dict[str, dict[str, str] | None] = {"value": None}
clear_feedback_lock = Lock()


def _is_connection_quota_error(exc: Exception) -> bool:
    """Detect local emulator quota errors to provide an actionable hint."""
    message = str(exc).lower()
    return "connectionsquotaexceeded" in message or "resource-limit-exceeded" in message
...
def home(request: Request) -> HTMLResponse:
    ...
    with clear_feedback_lock:
        feedback = clear_feedback["value"]
        # Consume feedback after one render, similar to a flash message.
        clear_feedback["value"] = None

    if feedback:
        action_message = feedback.get("action_message")
        action_error = feedback.get("action_error")
    ...
    def _fetch_queue(queue_name: str) -> dict[str, object]:
        ...
        try:
            ...
            if _is_connection_quota_error(exc):
                queue_errors.append(
                    "Message peek failed: connection quota exceeded for local emulator. "
                    "Lower BUSWATCH_MAX_PARALLEL_QUEUE_FETCHES and refresh again."
                )
            else:
    ...
    if queues_to_refresh:
        max_workers = min(len(queues_to_refresh), settings.max_parallel_queue_fetches)
        # Refreshes are parallelized to reduce end-to-end page latency when many
        # queues are loaded for the first time.
        if max_workers == 1:
            for queue_name in queues_to_refresh:
                with queue_cache_lock:
                    queue_cache[queue_name] = _fetch_queue(queue_name)
        else:
...
def clear_queue(queue_name: str) -> RedirectResponse:
    ...
    try:
        valid_queues = set(settings.queue_names or reader.list_queues())
    except Exception:  # pragma: no cover - depends on local emulator state
        valid_queues = set()
    ...
    try:
        ...
        with clear_feedback_lock:
            clear_feedback["value"] = {
                "action_message": f"Cleared {cleared_count} message(s) from queue '{queue_name}'.",
                "action_error": "",
            }
    except Exception as exc:  # pragma: no cover - depends on local emulator state
        with clear_feedback_lock:
            clear_feedback["value"] = {
                "action_message": "",
                "action_error": f"Unable to clear queue '{queue_name}': {exc}",
            }
        # Keep redirect target constant to avoid user-influenced redirect URLs.
        return RedirectResponse(url="/", status_code=303)
