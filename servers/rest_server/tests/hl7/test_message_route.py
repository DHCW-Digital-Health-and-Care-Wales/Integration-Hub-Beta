...
from tests.hl7.helpers import VALID_ER7_MESSAGE, build_test_context, make_config


class MessageRouteTests(unittest.TestCase):
    def _client(self, **kwargs: object) -> tuple[TestClient, MagicMock, MagicMock]:
        context, sender, store, _ = build_test_context(**kwargs)  # type: ignore[arg-type]
        return TestClient(create_hl7_app(context)), sender, store
    ...
    def test_oversize_message_returns_400(self) -> None:
        client, sender, _ = self._client(config=make_config(max_request_size_bytes=10))
        response = client.post("/hl7MessageReceiver", json={"messageContent": VALID_ER7_MESSAGE})
