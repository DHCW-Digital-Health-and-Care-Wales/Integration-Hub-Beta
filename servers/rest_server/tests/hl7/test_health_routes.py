...
from rest_server.hl7.app import create_hl7_app
from tests.hl7.helpers import build_test_context, make_config


class HealthRouteTests(unittest.TestCase):
    def setUp(self) -> None:
        context, _, _, _ = build_test_context()
        self.app = create_hl7_app(context)
...
class SwaggerGatingTests(unittest.TestCase):
    def test_docs_available_in_dev(self) -> None:
        context, _, _, _ = build_test_context(config=make_config(environment="DEV"))
        client = TestClient(create_hl7_app(context))
        self.assertEqual(client.get("/openapi.json").status_code, 200)
        self.assertEqual(client.get("/docs").status_code, 200)

    def test_docs_absent_in_production(self) -> None:
        context, _, _, _ = build_test_context(config=make_config(environment="PRD"))
        client = TestClient(create_hl7_app(context))
