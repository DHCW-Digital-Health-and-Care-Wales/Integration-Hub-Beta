...
class IterationDialog(tk.Toplevel):
    ...
    def __init__(
        ...
        ttk.Label(self, text="Mode").grid(row=1, column=0, sticky="w", padx=8, pady=4)
        ttk.OptionMenu(
        ...
        self._step_label.grid(row=2, column=0, sticky="w", padx=8, pady=4)
        self._step_entry = ttk.Entry(self, textvariable=self._step)
        self._step_entry.grid(row=2, column=1, sticky="ew", padx=8, pady=4)
        ...
        self._pad_label.grid(row=3, column=0, sticky="w", padx=8, pady=4)
        self._pad_entry = ttk.Entry(self, textvariable=self._pad_width)
        self._pad_entry.grid(row=3, column=1, sticky="ew", padx=8, pady=4)

        self._values_label = ttk.Label(self, text="Values (one per line, cycled in order)")
        self._values_label.grid(row=4, column=0, columnspan=2, sticky="w", padx=8, pady=(8, 0))
        self._values_box = tk.Text(self, width=36, height=5)
        if existing and existing.mode == "list":
            self._values_box.insert("1.0", "\n".join(existing.values))
        self._values_box.grid(row=5, column=0, columnspan=2, sticky="ew", padx=8, pady=4)
        ...
        self._timestamp_label.grid(row=6, column=0, sticky="w", padx=8, pady=4)
        self._timestamp_entry = ttk.Entry(self, textvariable=self._timestamp_format)
        self._timestamp_entry.grid(row=6, column=1, sticky="ew", padx=8, pady=4)

        button_row = ttk.Frame(self)
        button_row.grid(row=7, column=0, columnspan=2, pady=8)
