"""Configuration for the SOAP Subscription Sender service.

Mirrors ``soap_sender/app_config.py`` with the queue fields replaced by
topic + subscription fields to match the ``hl7_subscription_sender`` pattern.
No MessageStoreClient — consistent with the existing subscription sender.
"""
...
class AppConfig:
    ...
    ingress_topic_name: str
    ingress_subscription_name: str
    ingress_session_id: str | None   # Optional for subscription receivers
    # SOAP endpoint
    ...
    ws_security_enabled: bool          # Reserved for WS-Security token injection
    # Health check
    ...
    def read_env_config() -> AppConfig:
        return AppConfig(
            ...
            ingress_topic_name=_read_required_env("INGRESS_TOPIC_NAME"),
            ingress_subscription_name=_read_required_env("INGRESS_SUBSCRIPTION_NAME"),
