...
class TestChemocareHandler(unittest.TestCase):
    ...
    def test_valid_chemocare_messages_return_success_ack(self) -> None:
        """Test valid Chemocare messages return successful ACK"""
        test_cases = [
        ...
        for message_type, message, expected_control_id in test_cases:
            with self.subTest(message_type=message_type, control_id=expected_control_id):
                ...
                with patch(ACK_BUILDER_ATTRIBUTE) as mock_builder:
                    ...
                    # Verify successful ACK was created with correct control ID
                    mock_instance.build_ack.assert_called_once_with(expected_control_id, ANY, "AA")
    ...
    def test_invalid_authority_code_returns_failure_ack(self, mock_logger: MagicMock) -> None:
        ...
        handler = ChemocareHandler(
            INVALID_AUTHORITY_CODE_MESSAGE, self.mock_sender, self.mock_audit_client, self.mock_validator
        )
    ...
    def test_invalid_message_type_returns_failure_ack(self, mock_logger: MagicMock) -> None:
        ...
        handler = ChemocareHandler(
            INVALID_MESSAGE_TYPE_MESSAGE, self.mock_sender, self.mock_audit_client, self.mock_validator
        )
