"""SOAP subscription sender client — identical to soap_sender_client.

Duplicated intentionally to keep services independently deployable and
testable without cross-service imports — consistent with how
``hl7_sender_client`` and ``hl7_subscription_sender_client`` are handled.
"""
...
class SOAPSubscriptionSenderClient:
    """HTTP client that wraps HL7 ER7 in a SOAP envelope and POSTs to an endpoint.

    Mirrors ``SOAPSenderClient`` in ``soap_sender`` — duplicated for service
    independence rather than shared via a library.
    """
    ...
    def __enter__(self) -> "SOAPSubscriptionSenderClient":
        return self
