...
def _load_hl7_type_maps(base_dir: str, base_prefix: str) -> Tuple[Dict[str, str], Dict[str, List[str]], Dict[str, str]]:
    ...
    for el in fields_root.findall(f"{xs}element"):
        name = el.get("name")
        type_name = el.get("type")
        if name and type_name:
            element_to_type[name] = type_name
    for el in types_root.findall(f"{xs}element"):
        name = el.get("name")
        type_name = el.get("type")
        if name and type_name:
            element_to_type[name] = type_name
    ...
    for ctype in types_root.findall(f"{xs}complexType"):
        ...
        cc = ctype.find(f"{xs}complexContent")
        if cc is not None:
...
@lru_cache(maxsize=8)
def _load_segment_sequences(base_dir: str, base_prefix: str) -> Dict[str, List[Tuple[str, int | str, int | str]]]:
    segments_path = os.path.join(base_dir, f"{base_prefix}_segments.xsd")
    tree = ET.parse(segments_path)
    root = tree.getroot()
    xs = "{http://www.w3.org/2001/XMLSchema}"

    seq_map: Dict[str, List[Tuple[str, int | str, int | str]]] = {}
    for ctype in root.findall(f"{xs}complexType"):
        type_name = ctype.get("name")
        if not type_name or not type_name.endswith(".CONTENT"):
            continue
