...
from adt_receiver.adt_mllp_server import AdtMllpServer
...
class AdtReceiverApplication:
    def __init__(self) -> None:
        ...
        self.PORT = int(os.environ.get("PORT", "3475"))

        signal.signal(signal.SIGINT, self._signal_handler)
        signal.signal(signal.SIGTERM, self._signal_handler)

        self._server: AdtMllpServer | None = None
    ...
    def start_server(self) -> None:
        ...
        self.validator = HL7Validator(
            app_config.hl7_version, app_config.sending_app, app_config.message_type
        )
    ...
    def stop_server(self) -> None:
        logger.info("Shutting down the ADT receiver server...")
        ...
        if self._server:
            ...
            logger.info("ADT MLLP server shut down.")

        if self._server_thread:
            self._server_thread.join()

        logger.info("ADT receiver shutdown complete.")
