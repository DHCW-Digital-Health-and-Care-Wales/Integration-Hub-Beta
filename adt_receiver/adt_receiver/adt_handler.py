...
class AdtHandler(AbstractHandler):
    def __init__(
        self,
        msg: str,
        ...
        super(AdtHandler, self).__init__(msg)
    ...
    def reply(self) -> str:
        try:
            ...
            logger.info("Received ADT message type: %s, Control ID: %s", message_type, message_control_id)

            self.validator.validate(msg)
            self.event_logger.log_validation_result(
                self.incoming_message, f"Valid ADT message - Type: {message_type}", is_success=True
            )

            self._send_to_service_bus(message_control_id)

            ack_message = self._create_ack(message_control_id, msg)
            self.event_logger.log_message_processed(self.incoming_message, "ACK generated successfully")
            logger.info("ACK generated for Control ID: %s", message_control_id)
            return ack_message
            ...
            error_msg = f"Unexpected error while processing ADT message: {e}"
            logger.exception(error_msg)
            self.event_logger.log_message_failed(self.incoming_message, error_msg)
            raise

    def _create_ack(self, message_control_id: str, msg: Message) -> str:
    ...
    def _send_to_service_bus(self, message_control_id: str) -> None:
        try:
            self.sender_client.send_text_message(self.incoming_message, None)
            logger.info("ADT message %s sent to Service Bus successfully", message_control_id)
        except Exception as e:
            logger.error("Failed to send ADT message %s to Service Bus: %s", message_control_id, str(e))
            raise
